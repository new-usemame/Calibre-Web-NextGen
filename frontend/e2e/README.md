...
At this revision the derived set is 190 of 250 local Python modules (the closure correctly picks
...
`cps/services/koreader_xpointer.py` entered through the annotation routes that call it. The shared sign-in pacing helpers `cps/rate_limits.py` entered through the login, Kobo and HTTP Basic sign-in paths. Its position hand-off `cps/services/koreader_position.py` entered through the
