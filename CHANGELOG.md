...
- **Marking a shelf for Kobo sync now actually works — and the new UI tells you what else it needs.** Turning on "Kobo sync" for a shelf does nothing until your account is also set to sync only selected shelves; until now nothing said so, so a device would keep paging through the entire library instead of settling on the shelf. The shelf page now says it right where you make the mark, with a one-click switch and a note that books off the shelf leave the device but stay in your library. Separately, switching that account setting on from the new UI skipped the step that removes already-synced books from the device — so even after following the advice, the whole library stayed on the reader. It no longer does, and that clean-up step itself no longer removes books that **are** on a Kobo-sync shelf: it was matching every synced book against any ordinary shelf you happened to own, so books you had selected could be pulled off the device along with the rest. Books on a Kobo-sync magic shelf are kept too. Reported by [@auspex](https://github.com/auspex) ([#866](https://github.com/new-usemame/Calibre-Web-NextGen/issues/866)).
