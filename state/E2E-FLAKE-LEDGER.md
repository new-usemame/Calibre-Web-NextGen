...
| 2026-09-25 | GitHub-hosted CI; `E2E Tests (SPA)`; PR #2300 run 36158108003 attempt 1 (no frontend code in the PR; 781 passed, 1 flaky) | `help-banner-kofi.spec.ts:290` — `Help menu renders the Ko-fi label from an authenticated translated catalog` | First attempt timed out after 10.6 s on `toBeVisible()` for the German label `Ko-fi unterstützen →` (element not found); Playwright's retry passed in 1.6 s. | **Unknown.** Plausibly the translated catalog arriving after the menu first rendered, but nothing preserved shows the order. | **OPEN** |
| 2026-09-25 | Same run | `sp2-display-options.spec.ts:153` (`book detail exposes imported name, tag disclosure, and semantic progress`, its `route` handler for `/api/v1/books/6`) | The test passed, but its route handler's `route.fetch` was still in flight when the test ended, raising an unhandled `route.fetch: Test ended.`; that error, not an assertion, turned the job red. | The handler awaits `route.fetch()` without the test waiting for the response it intercepts, so a late request outlives the test. Fix direction: wait for that response in the test, or `route.fallback()` once the test is done. | **OPEN** |
