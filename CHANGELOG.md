...
- **A book that fails to convert now still lands in your library.** If CWA could not convert an incoming book to your target format — a big or image-heavy PDF that ran out of time, a file with internals Calibre chokes on, a format needing a plugin you don't have — the book was dropped altogether: no entry in the library, and the file gone from the ingest folder. The log even said `Successfully processed` on its way past, so nothing suggested a book had gone missing until you noticed it wasn't there. A failed conversion says nothing about whether the file is a readable book, and the two settings that decline to convert — auto-convert switched off, or the format on your do-not-convert list — already import the original, so this was inconsistent as well as lossy. The original is now imported whenever a conversion fails. The log also names the full path of the copy it keeps, `/config/processed_books/failed/`, in place of "failed backup", which told you a copy existed but not where. Separately, the ingest service no longer reports that the processor "should have timed out internally" when a conversion overruns — there is no such internal limit; the Ingest Timeout in CWA Settings governs it, and the safety limit is three times that value. Reported by [@auspex](https://github.com/auspex) ([#1094](https://github.com/new-usemame/Calibre-Web-NextGen/issues/1094)).
