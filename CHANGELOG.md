...
- **A book's page no longer fails to open when its library's extra fields can't be read.** Every book page asks the library for its custom-column definitions — the extra fields you can add to a book in Calibre. If that lookup failed because the library was mid-write, its folder had moved, or the definitions table wasn't there yet, the whole page returned an error rather than simply leaving those fields out. Custom columns are supplementary to a book page, so the page now loads without them and notes the reason in the log ([#1153](https://github.com/new-usemame/Calibre-Web-NextGen/issues/1153)).
