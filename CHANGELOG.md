...
- **Shelf pages now warn you when a Kobo-sync mark isn't doing anything.** Marking a shelf for Kobo sync has no effect while your account is still set to sync your whole library to the device, and the notice meant to say so never appeared — the setting it checks was missing from the data the page loads. The explanation and its one-click "Sync only my selected shelves" button now show up as intended, on both ordinary and smart shelves. Originally reported by [@auspex](https://github.com/auspex) ([#866](https://github.com/new-usemame/Calibre-Web-NextGen/issues/866)).
